...
cd ..
# /opt/qemu/bin/
qemu-system-xtensa -nographic -no-reboot -machine esp32s3 -drive file=build/flash_image.bin,if=mtd,format=raw -serial file:output.log

# Print the full test log to the GitHub Actions output
